...
public class NinoStepdefs {
	...
	public void iGoToNinoPage() {
		...
		assertThat(ninoPage.getExampleHintMessage().matches("For example, 'QQ 12 34 56 C'"));
	}
	...
	public void theNationalInsuranceNumberSubmissionWillBeUnsuccessful() {
		ninoPage = new NinoPage(driver);
		assertThat(ninoPage.getErrorHeadingErrorMessage()).matches("Some questions have not been answered correctly:");
		assertThat(ninoPage.getErrorsBelowErrorMessage()).matches("Please see the errors below.");
	}
	...
	public void iEnterNationalInsuranceNumberUsingTheNino(String nino) {
		ninoPage = new NinoPage(driver);
